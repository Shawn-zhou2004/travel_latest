...
| 模块 | 状态 |
|---|---|
| AI 行程规划 / 分域 RAG / 流式助手 / 会员与额度 / 订单与消息可靠性 | 完整实现，本地可跑通（需配置对应 AI 与基础设施 Key） |
| 支付宝支付 | 沙箱环境联调通过，RSA2 签名验签为真实实现（联调见 `docs/支付宝沙箱联调说明.md`） |
| 供应商履约（交通查询 / 出票确认） | 契约与状态机完整、幂等与死信兜底按真实标准实现；出票走 mock transport，未授权时安全降级（默认关闭，`SUPPLIER_INTEGRATION_ENABLED`） |
| 服务商工作台 | 后端 providers 模块（目录 / 工作台 / 服务管理 API）完整；配套管理端页面为规划项 |
| C 端体验模块 | 体验列表 + 详情页 + 预订下单链路可用 |

> 第三方供应商接入需要商务签约，按行业惯例以可替换的 transport 层先行打通链路，架构上换真实接入只需替换 transport 配置，业务代码零改动。
