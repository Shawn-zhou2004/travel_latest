...
export function normalizeApiError(error: unknown): ApiError {
  const existingCode = error instanceof Error ? (error as Partial<ApiError>).code : undefined
  if (typeof existingCode === 'string' && !existingCode.startsWith('ERR_')) return error as ApiError
...
export async function refreshAccessToken(baseURL = '/api/v1') {
  try {
    const { data } = await axios.post<{ access_token: string; user: SessionUser }>(`${baseURL}/auth/sessions/refresh`, undefined, { withCredentials: true })
    saveSession({ accessToken: data.access_token, audience: data.user.roles.includes('platform_admin') ? 'admin' : 'consumer', user: data.user })
    return true
  } catch {
    return false
  }
